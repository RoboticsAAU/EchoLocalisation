...
number = = 0;
directionSub = rossubscriber("/" + string(robot_id) + "/direction",'std_msgs/Int8',@directionCallback);
...
% t = 0;
% while t < 30
%     msg.X = currentPosition(1);
%     msg.Y = currentPosition(2);
%     msg.Theta = currentDirection;
...
dist = 0;
while dist < 10.0
    dist = norm(currentPosition) - norm(tmp)
...
function odomCallback(src,message)
   ...
   quaternionInput = message.Pose.Pose.Orientation;
   quat = [quaternionInput.X, quaternionInput.Y, quaternionInput.Z, quaternionInput.W];
   eul_orient = quat2eul(quat,"ZYX");
   currentDirection = eul_orient(1,3);
   
   tmpCurrentPosition = message.Pose.Pose.Position;
   currentPosition = [tmpCurrentPosition.X, tmpCurrentPosition.Y];
end
